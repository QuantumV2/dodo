...
### Buffers
Buffers are multiple ints enclosed by `{` (6) and `}` (7). When they are pushed by a variable, the brackets are excluded.
...
| LEN | 5 | Push stack length onto the stack
...
| LABEL | 28 | Pops a string off the stack and creates a label at the current position of the same name. |
| DELLABEL | 29 | Pops a string off the stack and murders a label of the same name. |

### Jump Ops
| Instruction | Opcode | Description |
|:------------|:------:|:------------|
| GOTO | 40 | Jumps to the label pointed to by the preceding string. E.g. `"MYLABEL" GOTO DO` | 
